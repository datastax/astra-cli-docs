...
Use the {product} to manage your {product-short} databases.
...
It cannot create {db-classic}; however, you can use the {product} to <<get-database-details,get information>> about your existing {db-classic}.
...
{product-short} databases store data in <<manage-collections,collections>> and <<manage-tables,tables>>, which exist inside keyspaces.
...
If you delete the default keyspace, {product-short} automatically selects another keyspace to be the default keyspace.
...
== Get a list of available {product-short} regions
...
Use the `astra db list-regions-classic` command to get a list of available cloud provider regions that support {db-classic}:
