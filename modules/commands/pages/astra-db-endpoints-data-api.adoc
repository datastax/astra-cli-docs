= astra db endpoints data-api

[source,shell]
----
astra db endpoints data-api [-cqV] [--no-input] [--[no-]spinner] [--color
                            [=WHEN]] [--dump-logs[=FILE]] [-h[=all]]
                            [-o=FORMAT] [-r=REGION] [[[-cf=PATH] [-p=NAME]] |
                            [--token=TOKEN [--env=ENV]]] [DB]

----


Get the Data API endpoint for the specified database
...
[source,bash]
----
# Get the Data API endpoint for the database
$ astra db endpoints data-api mydb

# Get the Data API endpoint for the database in a specific region
$ astra db endpoints data-api mydb --region us-east1
----
