...
astra db dsbulk unload [-qV] [--header] [--no-input] [--[no-]spinner] [--color
                       ...
                       [--delimiter=DELIMITER] [--dsbulk-config=CONFIG_FILE]
                       [--encoding=ENCODING] [-k=KEYSPACE]
                       [--log-dir=DIRECTORY] [-m=MAPPING]
...
`ARGS`:: Verbatim arguments to pass to dsbulk directly (anything after '--' is passed through)
