= astra db endpoints swagger

[source,shell]
----
astra db endpoints swagger [-cqV] [--no-input] [--[no-]spinner] [--color
                           [=WHEN]] [--dump-logs[=FILE]] [-h[=all]] [-o=FORMAT]
                           [-r=REGION] [[[-cf=PATH] [-p=NAME]] | [--token=TOKEN
                           [--env=ENV]]] [DB]
...
$ astra db endpoints swagger mydb

# Get the SwaggerUI endpoint for the database in a specific region
$ astra db endpoints swagger mydb --region us-east1
----
