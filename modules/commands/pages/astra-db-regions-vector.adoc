...
[source,shell]
----
astra db regions vector [-aqV] [--no-input] [--[no-]spinner] [--color[=WHEN]]
                        [--dump-logs[=FILE]] [-h[=all]] [-o=FORMAT] [-c=FILTER[,
                        FILTER...]]... [-f=FILTER[,FILTER...]]... [-z=FILTER[,
                        FILTER...]]... [[[-cf=PATH] [-p=NAME]] | [--token=TOKEN
                        [--env=ENV]]]
...
`-a`, `--all`:: List all available regions, including those not enabled for the current organization
...
$ astra db regions vector --cloud AWS,GCP

# Filter by partial region name
$ astra db regions vector --filter us-,ca-

# Filter by zone
$ astra db regions vector --zone Europe
----
