...
astra config create [-dqV] [--no-input] [--[no-]overwrite] [--[no-]spinner] [--
                    [no-]validate] [--color[=WHEN]] [--dump-logs[=FILE]] [-h
                    [=all]] [-cf=PATH] [-e=ENV] [--local-endpoint=URL]
...
`-e`, `--env`:: Astra environment the token belongs to: prod (default), dev, test, or local. Leave unset unless you were issued a non-prod token.
`--local-endpoint`:: The endpoint URL for local Astra environments (required when --env local)
...
`--validate`:: Validate the token by making a request to the Astra
