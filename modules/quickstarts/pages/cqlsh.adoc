...
The {product} doesn't download the `cqlsh` utility until the first time you use an `astra db cqlsh` command.
When this happens, the {product} downloads and installs the `cqlsh` utility, and the {scb} for each database you connect to, into the {product} home directory (`xref:ROOT:manage-cli.adoc#file-locations[~/.astra]`).
