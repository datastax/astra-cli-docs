...
:navtitle:  Issue CQL commands with cqlsh

The {cass-short} Query Language Shell (`cqlsh`) is a utility that you can use to issue xref:astra@cql:reference:cql-quick-reference.adoc[CQL commands] to your {product} database.

The {product} provides embedded `cqlsh` support by downloading, installing, configuring, and wrapping the `cqlsh` utility.
...
The first time you use a `astra db cqlsh` command, the {product} downloads and installs the `cqlsh` utility to the {product} home directory (`xref:ROOT:manage-cli.adoc#file-locations[~/.astra]`).
...
[source,shell,subs="+quotes"]
----
astra db cqlsh exec **DB_ID** "SOURCE 'sample.cql'"
----
...
----
 id                                   | email             | name
--------------------------------------+-------------------+-------
 f497124c-0095-4ba6-84e7-c704749e3927 |   bob@example.com |   Bob
 93b9fe1b-b8c8-4492-b26e-7a2467e73005 | alice@example.com | Alice
