...
For example, run `DESCRIBE KEYSPACES`:
...
The result of `DESCRIBE KEYSPACES` is printed to the terminal:
...
For example, using the following `sample.cql` file:
...
----
+
The `cqlsh` prompt shows the selected keyspace:
+
[source,console]
----
Connected to cndb at 127.0.0.1:9042.
...
----
+
The result of the `DESC TABLES` command is printed to the terminal:
+
[source,console]
----
