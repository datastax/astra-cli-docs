...
. When prompted, enter your application token (`AstraCS:...`), and then press kbd:[Enter]:
+
[source,console]
----
...
All subsequent commands use this profile unless you <<specify-configuration-profile-individual-command,override the configuration profile>>.
+
[source,console]
----
...
For example, the `list-clouds` command returns a list of cloud providers:
+
[source,console]
----
┌─────────────────┐
...
----
+
The output lists available commands and general help information:
+
[source,console]
----
     _____            __
...
----
+
The output is based on the specified command:
+
[source,console]
----
...
----
+
The output is based on the specified subcommand:
+
[source,console]
----
...
The output shows the list of configuration profiles.
...
The output shows the new profile name and a hint to set it as your default configuration profile:
...
The output confirms that the default profile has been set:
...
The output shows that the profile was deleted:
...
[cols=4]
|===
|File or Folder |Description |Default location |Contents

|`.astra`
|The {product} home folder (`ASTRA_HOME`)
a|
...
a|The {product} home folder contains the following items:
...
For more information, see the https://github.com/datastax/astra-cli?tab=readme-ov-file#astrarc-location[{product} README].
