...
Most commands support the `-o` / `--output` option to specify the output format:
...
Generally, for GET requests, the `json` response contains the raw response from the server, while the `human` and `csv` formats contain a simplified version of the data.
...
May contain commas or newlines — in such cases it will be quoted according to RFC-4180.

|Additional data fields
|Any (type varies by command)
a|Command-specific fields representing the actual data returned.

Fields may be empty if there is nothing to show.
