...
. When prompted `Are you sure you want to entirely delete Astra CLI from your system?`, select kbd:[yes], and then press kbd:[Enter]:
+
[source,console]
----
...
. When prompted, select whether to delete the `xref:ROOT:manage-cli.adoc#file-locations[.astrarc]` file.
+
Only select kbd:[yes] if you _don't_ want to preserve your xref:ROOT:manage-cli.adoc#manage-configuration-profiles[configuration profiles].
+
[source,console]
----
...
. Press kbd:[Enter] to proceed with the `astra nuke` operation.
+
Progress is reported in the terminal.
If you chose to keep the `.astrarc` file, the output shows that the file was skipped.
+
[source,console]
----
