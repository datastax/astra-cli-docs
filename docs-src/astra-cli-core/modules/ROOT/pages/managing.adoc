...
----
REGION OK
[INFO]  Database 'my_serverless_vector_db' does not exist. Creating database 'my_serverless_vector_db' with keyspace 'default_keyspace'
get CLoud provider
[INFO]  Enabling vector search for database my_serverless_vector_db
[INFO]  Database 'my_serverless_vector_db' and keyspace 'default_keyspace' are being created.
[INFO]  Database 'my_serverless_vector_db' has status 'PENDING' waiting to be 'ACTIVE' ...
[INFO]  Database 'my_serverless_vector_db' has status 'ACTIVE' (took 142496 millis)
[OK]    Database 'my_serverless_vector_db' is ready.
----
====
...
You can use the `astra db list` command to monitor the status of the database.
=====
...
Use the `astra db list` command to get a list of all databases in your organization:
...
| id               | 22fa1177-e365-4e20-90e6-7639664c9c6d    |
| Cloud            | AWS                                     |
| Regions          | us-east-1                               |
| Status           | ACTIVE                                  |
| Vector           | Disabled                                |
| Default Keyspace | my_keyspace                             |
| Creation Time    | 2025-01-06T20:34:35Z                    |
|                  |                                         |
| Keyspaces        | [0] my_keyspace                         |
|                  |                                         |
|                  |                                         |
| Regions          | [0] us-east-1                           |
|                  |                                         |
...
Use the `astra db status` command to get the status of a database:
...
Use the `astra db list-regions` command to get information about the regions that a database is deployed in:
...
Use the `astra db delete` command to delete an {astra_db} Serverless database:
...
Use the `astra db list-keyspaces` command to list the keyspaces in a database:
...
Use the `astra db delete-keyspace` command to delete a keyspace:
...
Use the `astra db unload` command to unload database table rows to a file:
